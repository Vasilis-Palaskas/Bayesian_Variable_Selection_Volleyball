...
data {
  ...
  int<lower=1> K;       // number of candidate variables for home team
  matrix[n_games, K] X_home_diff;   // design matrix for (home teams-away teams) skills
  matrix[n_games, K] X_away_diff;   // design matrix for (home teams-away teams) skills
...
transformed parameters {
  ...
  lambda1= exp(mu+home+(X_home_diff) * beta);          
  lambda2= exp(mu+(X_away_diff) * beta);    
  for (g in 1:n_games) {
...
model {
  ...
  //Priors
  target+=normal_lpdf(beta|0,1*c_std);
