...
transformed parameters {
  ...
  // Linear predictor
  lambda1= exp(mu+home+attack[home_team]+defense[away_team]+X_home * gb_home);          
  lambda2= exp(mu+attack[away_team]+defense[home_team]+X_away* gb_away); 
   ...
   for (i in 1:n_games) {
     for (j in 1:K){
      ...
      if (gb_home[j]==0){
         ...
         lambda1_new[i]=exp(mu+home+X_home[i,]*gb_new_home+attack[home_team[i]]+defense[away_team[i]]);
	...
	// We specified an upper bound in order to avoid numeric overflow problem for model parameters
      ...
      if (gb_away[j]==0){
         ...
         lambda2_new[i]=exp(mu+X_away[i,]*gb_new_away+attack[away_team[i]]+defense[home_team[i]]);
