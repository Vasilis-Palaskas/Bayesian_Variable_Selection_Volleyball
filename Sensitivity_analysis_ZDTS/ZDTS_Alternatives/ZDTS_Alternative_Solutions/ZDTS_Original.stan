...
transformed parameters {
  ...
  // Creation of linear predictor of Skellam model (used also by us for the ZDTS)
  ...
  for (g in 1:n_games){
  nomin[g]= (
    modified_bessel_first_kind(abs(1), 2 * sqrt(lambda1_star[g]*lambda2_star[g]))+
    +(2*modified_bessel_first_kind(abs(2), 2 * sqrt(lambda1_star[g]*lambda2_star[g]))*
    (lambda1_star[g]+lambda2_star[g])/
    sqrt(lambda1_star[g]*lambda2_star[g])
    )+(3*modified_bessel_first_kind(abs(3), 2 * sqrt(lambda1_star[g]*lambda2_star[g]))*
    (lambda1_star[g]^2+lambda1_star[g]*lambda2_star[g]+lambda2_star[g]^2)/
        (lambda1_star[g]*lambda2_star[g])
    )
    );
    
    denom[g]=(modified_bessel_first_kind(abs(1), 2 * sqrt(lambda1_star[g]*lambda2_star[g]))*
    (lambda1_star[g]+lambda2_star[g])+
   ( (modified_bessel_first_kind(abs(2), 2 * sqrt(lambda1_star[g]*lambda2_star[g]))*
    (lambda1_star[g]^2+lambda2_star[g]^2))/
     sqrt(lambda1_star[g]*lambda2_star[g])   )+ 
     (modified_bessel_first_kind(abs(3), 2 * sqrt(lambda1_star[g]*lambda2_star[g]))*
    (lambda1_star[g]^3+lambda2_star[g]^3) )/
     (lambda1_star[g]*lambda2_star[g])
     )   ;
  // Expected values components l1, l2 for the ZDTS distribution
     lambda1[g]=lambda1_star[g]*(nomin[g]/denom[g]);
     lambda2[g]=lambda2_star[g]*(nomin[g]/denom[g]);
  }
...
model {
  ...
  //Priors
  target+=normal_lpdf(beta_home|0,5);
  target+=normal_lpdf(beta_away|0,5);
