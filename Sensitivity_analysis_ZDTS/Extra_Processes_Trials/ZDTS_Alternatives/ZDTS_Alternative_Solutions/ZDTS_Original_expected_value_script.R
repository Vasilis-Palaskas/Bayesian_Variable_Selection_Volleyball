...
data_zdts_skills_original<-list(n_games=data_zdts_skills$N,
...
full_zdts_skills_original<-stan(file="zdts_Original.stan",
                       data=data_zdts_skills_original,thin=1,chains=1,cores=1,
                       iter=12000,warmup=2000,seed="12345",init_r=1)#15948/16000=99.6% divergent transitions after warmup

save(full_zdts_skills_original,file="full_zdts_skills_original")
##---Deviances
dev_full_zdts_skills_original<-extract(full_zdts_skills_original,pars="dev")
dev_full_zdts_skills_original$dev
####----- Posterior distributions of betas parameters across several c values
####-----  c: prior standard deviation multiplicator for betas parameters


min(dev_full_zdts_skills_original$dev)#
sd(dev_full_zdts_skills_original$dev)
rstan::check_divergences(full_zdts_skills_original)#0%

# ###-----------Rename the coefficients of stan fit objects
beta_home_full_zdts_skills_original<-extract(full_zdts_skills_original,pars="beta_home")
beta_away_full_zdts_skills_original<-extract(full_zdts_skills_original,pars="beta_away")

colnames(beta_home_full_zdts_skills_original)<-c(
  "(Home) perfect serve","(Home) very good serve","(Home) failed serve","(Home) perfect pass",
  "(Home) very good pass","(Home) poor pass","(Home) failed pass","(Home) perfect att1","(Home) blocked att1",
  "(Home) failed att1","(Home) perfect att2","(Home) blocked att2","(Home) failed att2","(Home) perfect block",
  "(Home) block net violation","(Home) failed block","(Home) failed setting")

colnames(beta_away_full_zdts_skills_original)<-c(
...
###---
names(full_zdts_skills_original)[c(1:17)]<-c(
  "(Home) perfect serve","(Home) very good serve","(Home) failed serve","(Home) perfect pass",
  "(Home) very good pass","(Home) poor pass","(Home) failed pass","(Home) perfect att1","(Home) blocked att1",
  "(Home) failed att1","(Home) perfect att2","(Home) blocked att2","(Home) failed att2","(Home) perfect block",
  "(Home) block net violation","(Home) failed block","(Home) failed setting")

names(full_zdts_skills_original)[c(18:34)]<-c(
  "(Away) perfect serve","(Away) very good serve","(Away) failed serve","(Away) perfect pass",
  "(Away) very good pass","(Away) poor pass","(Away) failed pass","(Away) perfect att1","(Away) blocked att1",
  "(Away) failed att1","(Away) perfect att2","(Away) blocked att2","(Away) failed att2","(Away) perfect block",
  "(Away) block net violation","(Away) failed block","(Away) failed setting")
...
full_zdts_skills_original_summary<-summary(full_zdts_skills_original,
                                  pars=c("beta_home","beta_away"))

posterior_summary_betas_zdts_skills<-round(
  cbind(full_zdts_skills_original_summary$summary[,c(1,3)],full_zdts_skills_original_summary$summary[,c(1,3)],
        full_zdts_skills_original_summary$summary[,c(1,3)],full_zdts_skills_original_summary$summary[,c(1,3)]),2)



##4) Count how many times exceeded these values
lambda1_star_full_zdts_skills_original<-extract(full_zdts_skills_original,pars="lambda1_star")
lambda1_star_full_zdts_skills_original$lambda1_star

lambda2_star_full_zdts_skills_original<-extract(full_zdts_skills_original,pars="lambda2_star")
lambda2_star_full_zdts_skills_original$lambda2_star

array_posterior_full_zdts_skills_original<-as.array(full_zdts_skills_original)

### 95% Posterior Density Areas for skills parameters across all candidate models (c=4,25,100 in variances)
plot_beta_home_posterior_full_zdts_skills_original<-mcmc_areas(beta_home_full_zdts_skills_original,
...
plot_beta_away_posterior_full_zdts_skills_original<-mcmc_areas(beta_away_full_zdts_skills,
                                                          prob = 0.95,prob_outer=0.95,
...
full_zdts_skills_original<-ggarrange(plot_beta_home_posterior_full_zdts_skills_original, plot_beta_away_posterior_full_zdts_skills,
                                         plot_beta_home_posterior_full_zdts_skills, plot_beta_away_posterior_full_zdts_skills,
                                         ncol = 2, nrow = 2)
...
mcmc_beta_home_full_zdts_skills_original<-as.mcmc(beta_home_full_zdts_skills_original)
mcmc_beta_away_full_zdts_skills_original<-as.mcmc(beta_away_full_zdts_skills_original)


##--- MCMC Convergence Diagnostics
##--- Autocorrelation, Trace and Cumsum Plots
autocorr.plot(mcmc_beta_home_full_zdts_skills_original)
autocorr.plot(mcmc_beta_away_full_zdts_skills_original)


traceplot(mcmc_final_posterior_values_betas_home)
traceplot(mcmc_beta_away_full_zdts_skills_original)


cumsumplot(mcmc_beta_home_full_zdts_skills_original)
cumsumplot(mcmc_beta_away_full_zdts_skills_original)
